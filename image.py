...
def save_img():
    ...
    return_name  = request.url+'/img/'+str(time.strftime('%m', time.localtime(time.time())))+'/'+str(time.strftime('%d', time.localtime(time.time())))\
            +'/'+image_name+'.jpg'
...
def save_img_end():
    ...
    return_name  =  request.url+'/img_end/'+str(time.strftime('%m', time.localtime(time.time())))+'/'+str(time.strftime('%d', time.localtime(time.time())))\
            +'/'+image_name+'.jpg'
    result_text=json.dumps({'code':200,'file':return_name})
    rst = make_response(result_text)
    rst.headers['Access-Control-Allow-Origin'] = '*'
